...
Carte::Carte(const std::vector<std::string>& texte,
             const std::vector<std::vector<int>>& gain)
...
type_case Carte::get_case(int x, int y) const {
  if (!case_valide(x, y)) return CASE_INVALIDE;
  return grille_[y][x];
}

void Carte::set_case(int x, int y, type_case nouvelle_case) {
  if (!case_valide(x, y)) return;
...
bool Carte::ile_presente(int x, int y) const {
  if (!emplacement_valide(x, y)) return false;
