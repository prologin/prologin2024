...
TEST_F(ApiTestColibri, TestApiRecupererTerritoire) {
  ...
  for (auto& p : api->recuperer_territoire(0)) {
    if (p.colonne == pos1.colonne && p.ligne == pos1.ligne) b1 = true;
    if (p.colonne == pos2.colonne && p.ligne == pos2.ligne) b2 = true;
    if (p.colonne == pos3.colonne && p.ligne == pos3.ligne) b3 = true;
  }
  EXPECT_TRUE(b2);
  EXPECT_TRUE(b1);
  EXPECT_TRUE(b3);
