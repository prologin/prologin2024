...
void Rules::apply_action(const rules::IAction& action) {
  ...
  if (err != 0)
    FATAL(
        "Synchronization error: received action %d from player %d, but "
...
void Rules::spectator_turn() { champion_jouer_tour_(); }
...
void Rules::end_of_round() { api_->game_state().sync_score(); }
