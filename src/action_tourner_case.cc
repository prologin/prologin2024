...
bool case_bloquee(const std::vector<Aigle>& aigles, int x, int y) {
  for (const Aigle& aigle : aigles) {
    if (aigle.effet != EFFET_GEL) continue;
    if (aigle.case_dans_rayon(x, y)) return true;
  }
  return false;
}
...
int ActionTournerCase::check(const GameState& st) const {
  ...
  const Carte& carte = st.carte;

  int x = pos_.colonne;
  int y = pos_.ligne;

  if (!carte.case_valide(x, y)) return POSITION_INVALIDE;

  if (case_bloquee(st, x, y)) return CASE_BLOQUEE;

  if (carte.get_case(x, y) == VILLAGE) return ROTATION_VILLAGE;

  const Joueur& joueur = st.joueurs[player_id_];
  const Joueur& adversaire = st.joueurs[player_id_ ^ 1];
  const std::vector<std::vector<bool>> territoire_adverse =
      adversaire.territoire(st.carte);
  ...
  for (int emplacement_y = y - 1; emplacement_y <= y; emplacement_y++) {
    for (int emplacement_x = x - 1; emplacement_x <= x; emplacement_x++) {
      if (!carte.emplacement_valide(emplacement_x, emplacement_y)) continue;
      if (territoire_adverse[emplacement_y][emplacement_x])
        cout = COUT_ROTATION_ENNEMI;
    }
  }

  if (joueur.points_action < cout) return PLUS_DE_PA;
...
void ActionTournerCase::apply_on(GameState* st) const {
  ...
  std::vector<std::vector<bool>> territoire_adverse =
      adversaire.territoire(carte);
  int cout = COUT_ROTATION_STANDARD;
  for (int emplacement_y = y - 1; emplacement_y <= y; emplacement_y++) {
    for (int emplacement_x = x - 1; emplacement_x <= x; emplacement_x++) {
      if (!carte.emplacement_valide(emplacement_x, emplacement_y)) continue;
  ...
  carte.set_case(x, y, rotation_case(actuel));
