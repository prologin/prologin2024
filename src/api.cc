...
etat_case Api::info_case(position pos) {
  int x = pos.colonne;
  int y = pos.ligne;
  if (!game_state_->carte.case_valide(x, y)) return {CASE_INVALIDE, 0, pos};
...
std::vector<aigle> Api::info_aigles() {
  ...
  for (Aigle& aigle_sauvage : game_state_->aigles_sauvages) {
    const aigle aigle_converti = {
        aigle_sauvage.identifiant, -1,
        aigle_sauvage.pos,         aigle_sauvage.effet,
        aigle_sauvage.puissance,   aigle_sauvage.tour_eclosion};
  ...
  for (int joueur = 0; joueur <= 1; joueur++) {
    for (Aigle& aigle_joueur : game_state_->joueurs[joueur].aigles) {
      aigle aigle_converti = {
          aigle_joueur.identifiant, joueur,
          aigle_joueur.pos,         aigle_joueur.effet,
          aigle_joueur.puissance,   aigle_joueur.tour_eclosion};
      aigles.push_back(aigle_converti);
    }
  }
...
int Api::moi() { return player_->id; }

int Api::adversaire() { return 1 - player_->id; }
...
int Api::tour_actuel() { return game_state_->tour; }

std::vector<position> Api::liste_villages(int joueur) {
  if (joueur == -1) return game_state_->villages_libres;
  if (joueur > 1 || joueur < -1) return std::vector<position>();
  return game_state_->joueurs[joueur].villages;
}

std::vector<position> Api::recuperer_territoire(int joueur) {
  if (joueur < 0 || joueur > 1) return std::vector<position>();

  const Carte& carte = game_state_->carte;
  std::vector<std::vector<bool>> territoire =
      game_state_->joueurs[joueur].territoire(carte);

  const auto [largeur, hauteur] = game_state_->carte.get_dimension();

  std::vector<position> resultat;
  for (int y = 0; y < hauteur - 1; y++) {
    for (int x = 0; x < largeur - 1; x++) {
      if (territoire[y][x]) resultat.push_back({x, y});
    }
  }
...
void Api::afficher_erreur(erreur v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, type_case v);
void Api::afficher_type_case(type_case v) { std::cerr << v << std::endl; }
...
void Api::afficher_type_action(type_action v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, effet_aigle v);
void Api::afficher_effet_aigle(effet_aigle v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, position v);
void Api::afficher_position(position v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, dimension v);
void Api::afficher_dimension(dimension v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, aigle v);
void Api::afficher_aigle(aigle v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, etat_case v);
void Api::afficher_etat_case(etat_case v) { std::cerr << v << std::endl; }

std::ostream& operator<<(std::ostream& os, action_hist v);
void Api::afficher_action_hist(action_hist v) { std::cerr << v << std::endl; }
