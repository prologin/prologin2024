...
void GameState::capture(int largeur, int hauteur, int j_actuel_id,
  ...
  const Joueur& j_adverse = joueurs[(j_actuel_id + 1) % 2];
  ...
  auto const verifie_territoire = [&territoire, &territoire_adverse,
                                   tour_actuel](const Aigle& aigle) {
        ...
        for (int dx = 0; dx >= -1; dx--) {
          for (int dy = 0; dy >= -1; dy--) {
            ...
            if (0 <= rx && rx < largeur - 1 && 0 <= ry && ry < hauteur - 1) {
              if (territoire_adverse[ry][rx]) return false;
              if (territoire[ry][rx]) dans_mon_territoire = true;
            }
          }
        }
  ...
  if (!aigles_sauvages.empty()) {
    ...
    aigles_sauvages.erase(
        std::remove_if(aigles_sauvages.begin(), aigles_sauvages.end(),
                       verifie_territoire),
        aigles_sauvages.end());
  }
  ...
  if (villages_libres.size() > 0) {
    ...
    villages_libres.erase(
        std::remove_if(villages_libres.begin(), villages_libres.end(),
                       verifie_deux_territoires),
        villages_libres.end());
  }
}
...
GameState::GameState(const rules::Players& players, std::ifstream& json_file)
  ...
  for (const auto& ligne : donnees["gains"]) {
    ...
    for (int elt : ligne) ligne_vec.push_back(elt);
  ...
  for (const auto& ligne : donnees["carte"]) carte_texte.push_back(ligne);
  ...
  for (int y = 0; y < hauteur; y++) {
    for (int x = 0; x < largeur; x++) {
      if (carte.get_case(x, y) != VILLAGE) continue;
      if (x == villages1[0].colonne && y == villages1[0].ligne) continue;
      if (x == villages2[0].colonne && y == villages2[0].ligne) continue;
...
bool GameState::est_termine() const { return tour > NB_TOURS; }

int calcul_multiplicatif(const std::vector<Aigle>& aigles, int x, int y) {
  int multiplicatif = 1;
  for (const Aigle& aiglantine : aigles) {
    if (aiglantine.effet != EFFET_FEU) continue;
    if (aiglantine.pos.colonne != x || aiglantine.pos.ligne != y) continue;
...
bool GameState::annuler() {
  if (historiques[joueur_actuel()].empty()) return false;
...
json dump_debug(const Carte& carte,
  ...
  for (int y = 0; y < hauteur; y++) {
    json jligne = json::array();
    for (int x = 0; x < largeur; x++) jligne.push_back(carte_debug[y][x]);
...
json dump_joueur(const Joueur& joueur) {
  ...
  for (const Aigle& aigle : joueur.aigles) jaigles.push_back(dump_aigle(aigle));
...
json dump_territoire(const GameState& st) {
  ...
  const auto [largeur, hauteur] = st.carte.get_dimension();
