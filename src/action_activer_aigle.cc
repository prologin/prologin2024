...
int ActionActiverAigle::check(const GameState& st) const {
  ...
  if (aiglantine == nullptr) return AIGLE_INVALIDE;

  if (aiglantine->effet != EFFET_METEORE && aiglantine->effet != EFFET_VIE &&
...
bool envoler_aigle_id(std::vector<Aigle>& aigles, int id) {
  ...
  if (it == aigles.cend()) return false;
  aigles.erase(it);
  return true;
}

void envoler_aigle_pos(std::vector<Aigle>& aigles, position pos,
                       int tour_actuel, int puiss) {
  const auto aigle_correct = [pos, tour_actuel, puiss](const Aigle& aigle) {
    return aigle.pos.colonne + puiss <= pos.colonne &&
           aigle.pos.colonne - puiss >= pos.colonne &&
           aigle.pos.ligne + puiss <= pos.ligne &&
           aigle.pos.ligne - puiss >= pos.ligne &&
           aigle.tour_eclosion <= tour_actuel;
  };
  const auto erase_iterator =
      std::remove_if(aigles.begin(), aigles.end(), aigle_correct);
  aigles.erase(erase_iterator, aigles.end());
}

void tourner_cases(GameState& st, Carte& carte, Aigle& aigle) {
  for (int x = aigle.pos.colonne - aigle.puissance;
       x <= aigle.pos.colonne + aigle.puissance + 1; x++) {
    for (int y = aigle.pos.ligne - aigle.puissance;
         y <= aigle.pos.ligne + aigle.puissance + 1; y++) {
...
void ActionActiverAigle::apply_on(GameState* st) const {
  ...
  st->ajoute_historique(
      {ACTION_ACTIVER_AIGLE, aiglantine->pos, aiglantine->pos, id_});
}
